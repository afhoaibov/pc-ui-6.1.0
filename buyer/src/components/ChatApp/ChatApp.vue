    ...
    <div class="chat-container">
      <div class="left-chat">
        ...
        <div class="friend-list">
          ...
          >
            <div v-if="!Number(item.target_id)" class="face-friend" :style="{backgroundImage: `url(${platformFace})`}"></div>
            <div v-else class="face-friend" :style="{backgroundImage: `url(${item.avatar || defaultFace})`}"></div>
      ...
      <div class="right-chat">
        <div class="head-right">
          <div v-if="currentChat" class="wrapper-head">
            <div v-if="!Number(currentChat.target_id)" class="face-head" :style="{backgroundImage: `url(${platformFace})`}"></div>
            <div v-else class="face-head" :style="{backgroundImage: `url(${currentChat.avatar || defaultFace})`}"></div>
            <div class="info-head">
            ...
            <div class="content-actions">
              <div v-show="current_tab === 0" class="history-actions" :class="{'is-seller': isSeller}">
                ...
                <div v-if="currentAskGoods" class="history-goods">
                  <router-link :to="`/goods/${currentAskGoods['goods_id']}`" target="_blank" class="history-goods-link">
                ...
                <div v-for="item in goodsList" :key="item['goods_id']" class="history-goods">
                  <router-link :to="`/goods/${item['goods_id']}`" target="_blank" class="history-goods-link">
                ...
                <div v-for="item in orders" :key="item['order_id']" class="item-order">
                  ...
                  <div class="other-order">
                    <p class="price-order">
                      <span>订单金额：</span>
                      <span class="price">
                        <en-price :price="item['order_amount']" direction="row" size="13" />
                      </span>
                    </p>
                    <div>
...
export default {
  name: 'ChatApp',
  components: { ChatAppEmoji, ChatMessageItem, Viewer },
  props: {
    user: {
      type: Object,
      required: true
  ...
  async created() {
    ...
    websocket.init(this.apis['buyer'] + '/ws/shop?appType=buyer&uuid=' + uuid, this.handleWebsocketMessage, accessToken)
    console.log(this.isSeller)
    console.log(shop_id)
    if (!this.isSeller) {
      if (shop_id) {
        this.goodsListParams.seller_id = shop_id
      }
      this.getGoodsList()
    }
    if (this.receiver_id) {
  ...
  methods: {
    ...
    handleClickFriend(item) {
      ...
      if (!this.isSeller) {
        ...
        this.goodsListParams.seller_id = item['target_id']
        this.getGoodsList()
      }
    },
    /**
    ...
    handleClickMessage(message) {
      if (message.type === 'ORDER') {
        ...
        const route = this.$router.resolve(`/goods/${message['goods_id']}`)
