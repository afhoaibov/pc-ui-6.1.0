  ...
  <div id="my-coupons">
    ...
    <div class="coupons-container">
      ...
      <ul v-else class="coupon-list">
        <li v-for="(coupon, index) in coupons.data" :key="index" class="coupon-item" :class="[coupon.used_status === 1 && 'used', coupon.is_cancel === 1 && 'overdue']">
          <div class="c-type">
            <div class="c-money">
              <strong>
                <!-- {{ coupon.coupon_price | unitPrice }} -->
                <en-price :price="coupon.coupon_price" priceColor="#fff" i18nPriceColor="#fff" size="40" />
              </strong>
            </div>
            <div class="c-limit">
              满<!-- {{ coupon.coupon_threshold_price | unitPrice }} -->
              <en-price :price="coupon.coupon_threshold_price" priceColor="#fff" i18nPriceColor="#fff" direction="row" :sizeRatio="0.9" size="12" />可用
            </div>
            <div class="c-shop" v-if="coupon.use_scope">
              仅限
            ...
            <div class="c-shop" v-else>
              仅限<router-link :to="'/shop/' + coupon.seller_id" style="color: #fff">{{ coupon.seller_name }} 店铺使用</router-link>
            </div>
            <div class="c-time">
          ...
          <div class="c-othr">
            <div v-if="coupon.used_status === 1">
              <p>订单号：{{ coupon.order_sn }}</p>
              <p>已使用</p>
            </div>
            <div v-else>
              <div v-if="coupon.is_cancel === 1">
                <p>已失效</p>
              </div>
              <div v-else>
                <router-link v-if="coupon.use_scope === 'ALL'" :to="'/goods'" class="use-btn">立即使用</router-link>
                <router-link v-else-if="coupon.use_scope === 'CATEGORY'" :to="'/goods?category=' + coupon.scope_id" class="use-btn">
                  立即使用
                </router-link>
                <router-link v-else-if="coupon.use_scope === 'SOME_GOODS'" :to="'/goods?sku_ids=' + coupon.scope_id" class="use-btn">
                  立即使用
                </router-link>
                <router-link v-else :to="'/shop/' + coupon.seller_id" class="use-btn">立即使用</router-link>
              </div>
            </div>
          </div>
          <!--<i class="is-used"></i>-->
...
export default {
  ...
  data() {
    return {
      ...
      tabs: [
        { label: '可用优惠券', key: 1, active: true },
        { label: '已用优惠券', key: 3, active: false },
        { label: '已失效优惠券', key: 4, active: false }
      ]
    }
  },
  mounted() {
  ...
  methods: {
    ...
    GET_Coupons() {
      API_Members.getCoupons({ ...this.params, status: this.tabs.find(item => item.active).key }).then(response => {
        this.coupons = response
        for (let i in this.coupons.data) {
          if (this.coupons.data[i].scope_id && this.coupons.data[i].use_scope === 'CATEGORY') {
            this.coupons.data[i].scope_id = this.coupons.data[i].scope_id.split(',')[1]
          }
          // 如果返回的use_scope为部分商品 SOME_GOODS
          if (this.coupons.data[i].use_scope === 'SOME_GOODS') {
            this.coupons.data[i].scope_id = encodeURIComponent(this.coupons.data[i].scope_id)
          }
        }
