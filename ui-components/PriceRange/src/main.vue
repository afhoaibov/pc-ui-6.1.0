...
<script>
  export default {
    ...
    created: function () {
      ...
      let _this = this
      this.defaultRange.forEach(function (item, index) {
        let preRange = _this.defaultRange[index - 1]
    ...
    methods: {
      handleSetPriceRange: function () {
        this.dialogVisible = true
      },
      /** 增加一个区间 */
      handleAddRange: function () {
      ...
      /** 删除一个区间 */
      handleDeleteRange: function () {
        // 如果还剩一个，则不删除
        if (this.priceRange.length <= 1) return
        this.priceRange.pop()
      },
      // 校验最后一个数是否符合要求
      validLastVal: function () {
        let priceRange = this.priceRange
        let last_range = priceRange[priceRange.length - 1]
