<template>
  <div class="ckt-item receipt">
    ...
    <div class="content-ckt receipt"
         v-if="isShowMembersReceip">
      <div v-if="!receipt || !receipt.receipt_title || !price"
           class="item">无需发票</div>
      <template v-else>
      ...
      <a href="javascript:"
         class="item edit-btn"
         @click="handleEditReceiptInfo">修改</a>
      <a v-if="receipt && receipt.receipt_title && !price"
         href="javascript:"
         class="item cancel-btn"
         @click="handleCancelReceipt">取消发票</a>
    </div>
    <div class="content-ckt receipt"
         v-if="!isShowMembersReceip">
      ...
      <div class="invoice-information">
        <ul style=""
          ...
          <li v-for="item in tabData"
              :key="item.name"
              :class="[item.show ? 'current' : null]"
              @click="selectNav(item)">
            {{ item.name }}</li>
        </ul>
        <ul class="invoice-content">
          <!-- 普通发票 -->
          <vatordinary v-if="tabData[0].show"
                       :receipt="receipt"
                       :tabData="tabData[0]"
                       ref="vatordinary"></vatordinary>
          <!-- 电子普通发票 -->
          <electro v-if="tabData[1].show"
                   :receipt="receipt"
                   :tabData="tabData[1]"
                   ref="electro"></electro>
          <!-- 增值税专用发票 -->
          <vatospecial v-if="tabData[2].show"
                       :receipt="receipt"
                       :tabData="tabData[2]"
                       :order-way="orderWay"
                       ref="vatospecial"></vatospecial>
        </ul>
      </div>
    </div>
  </div>
...
export default {
  ...
  props: ['receipt', 'inventoryList', 'orderWay'],
  data() {
    return {
      // 标签数据
      tabData: [
  ...
  methods: {
    setReceipt() {
      ...
      if (currentTabData.receipt_type === 'VATORDINARY') {
        // TODO
        this.$refs.vatordinary.handleConfirmReceipt(0, this)
      }
    },
    // 选取 对应的 发票内容
    selectContentMethod(itemContent) {
      this.content.forEach(item => {
        item.id === itemContent.id ? item.select = true : item.select = false
      })
    },
    // 切换 不同的导航菜单
    selectNav(item) {
      ...
      this.tabData.forEach(i => {
        i.name === item.name ? i.show = true : i.show = false
      })
    ...
    handleEditReceiptInfo() {
      if (!this.price || !this.goods_price) {
        this.$message.error('该订单不支持开具发票')
        return
      }
      this.$layer.open({
        type: 1,
    ...
    async queryMembersReceiptInfo() {
      let inventoryList = this.inventoryList
      this.price = inventoryList.some(item => item.price.original_price)
      this.goods_price = inventoryList.some(item => item.price.goods_price)
      let seller_id_arr = inventoryList.map(item => item.seller_id)
      if (seller_id_arr.length) {
        const params = { seller_ids: [...seller_id_arr] }

        const response = await API_Members.queryMembersReceipt(params)
        this.membersReceiptInfo = response
        this.tabData.forEach(item => {
          item.isChecked = !!response[item.key]
        })
      }
    }
  }
