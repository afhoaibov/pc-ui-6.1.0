          ...
          <li v-for="(item, index) in comments.data" :key="index">
            ...
            <div class="order-tbody-item">
              <div class="order-item-sku">
                <div class="sku-item" v-for="(sku, skuIndex) in item.sku_list" :key="skuIndex">
                  ...
                  <div class="goods-name-box">
                    <router-link :to="'/goods/' + sku.goods_id" class="goods-name" target="_blank" v-html="sku.name">
                      {{ sku.name }}
                    </router-link>
                  </div>
              ...
              <div class="order-item-status">
                ...
                <en-price :price="item.order_amount || 0" :sizeRatio="0.8" size="12" direction="row" />
              </div>
              <div class="order-item-operate">
                <router-link :to="'/member/comments?order_sn=' + item.sn">去评论</router-link>
              </div>
            </div>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    /** 获取待评价订单列表 */
    GET_Comments() {
      API_Order.getOrderList({ ...this.params, tag: 'WAIT_COMMENT' }).then(response => {
