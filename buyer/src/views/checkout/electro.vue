  ...
  <li>
    ...
    <el-form :model="receiptForm" :rules="receiptRules" ref="receiptForm" label-width="100px">
      <el-form-item label="发票抬头" prop="receipt_title">
        <!--个人抬头 start-->
        <div class="ckt-checkbox title-receipt"
          :class="[selectedReceipt && selectedReceipt.receipt_title === '个人' && 'selected']" @click="handleSelectReceipt">
          <span class="title">个人</span>
        </div>
        <!--个人抬头 end-->
        <!--个人发票抬头列表 start-->
        <div v-for="receipt in receipts" :key="receipt.receipt_title" class="ckt-checkbox title-receipt" :class="[
          ...
          <input class="company-receipt-input" v-model="receiptForm.receipt_title" maxlength="60" placeholder="单位发票抬头"
            @click.stop="() => { }">
        ...
        <!--个人发票抬头列表 end-->
        <!--编辑抬头 start-->
        <div v-if="receiptForm.receipt_title !== undefined && receiptForm.receipt_id === -1"
        ...
        <!--编辑抬头 end-->
        <!--新增单位发票 start-->
        <a v-if="!receiptForm.receipt_id" href="javascript:" class="add-receipt" @click="handleAddReceipt">新增单位发票</a>
        <!--新增单位发票 end-->
      </el-form-item>

      <el-form-item label="纳税人识别号" prop="tax_no"
        v-if="selectedReceipt === '' || (selectedReceipt && selectedReceipt.receipt_title !== '个人')">
        <input v-model="receiptForm.tax_no" class="duty-receipt-input" placeholder="请填写纳税人识别号" maxlength="50">
      </el-form-item>
      <el-form-item label="发票内容" prop="content">
        <div v-for="item in content" :key="item.id" @click="selectContentMethod(item)"
          :class="[item.select ? 'selected' : null, 'ckt-checkbox content-receipt']">
      ...
      <el-form-item label="收票人邮箱" prop="member_email">
        <el-input v-model="receiptForm.member_email" class="el-duty-receipt-input" placeholder="请填写收票人邮箱" />
      </el-form-item>
    </el-form>
  </li>
</template>
  ...
  methods: {
    init() {
      ...
      this.content.forEach(item => {
        item.select = item.label === this.selectedReceipt.receipt_content
      })
    },
    // 选取 对应的 发票内容
    selectContentMethod(itemContent) {
      this.content.forEach(item => {
        item.select = item.id === itemContent.id
      })
    ...
    handleConfirmReceipt(index, callback) {
      const { mobile_regex, areaValue_n, formatValid } = this.$refs.AreaSelect
      const receiptForm = cloneObj(this.receiptForm)
      if (!this.selectedReceipt.receipt_title) {
        this.$message.error('请选择发票抬头！')
        return false
      }
      if (!receiptForm.member_mobile) {
        this.$message.error('请填写收票人手机号！')
        return false
      } else if (!Foundation.eval2(mobile_regex).test(formatValid(receiptForm.member_mobile))) {
        this.$message.error('请输入正确的手机号!')
        return false
      }
      if (receiptForm.member_email) {
        if (!RegExp.email.test(receiptForm.member_email)) {
          this.$message.error('请输入正确的邮箱!')
          return false
        }
      }
      if (areaValue_n) {
        receiptForm.member_mobile = `${areaValue_n},${receiptForm.member_mobile}`
      }
      ...
      const receipt_type = this.tabData['receipt_type']
      API_Trade.setRecepit({ ...receipt, ...receiptForm, receipt_type }).then(() => {
