...
export default {
  ...
  methods: {
     ...
     * @constructor
     */
    GET_ChildrenById(code) {
      this.loading = true
      let api
      code ? api = this.api2 + '/' + code : api = this.api
      request({
        url: api,
        method: 'get',
        loading: false
    ...
    dataProcessing(res) {
      ...
      if (this.curLevel < maxLevel) {
        this.GET_ChildrenById(res[0]['$code'])
      } else {
        this.handleRturnItem()
      }
    },
    ...
    handleClickItem(item, itemIndex, columnIndex) {
      ...
      if (columnIndex < this.maxLevel - 1) {
        this.GET_ChildrenById(item.$code)
      } else {
        this.handleRturnItem()
      }
    },
    ...
    refresh(type, item) {
      ...
      if ($level === 0) {
        ...
        this.GET_ChildrenById(item.country_code)
      }
    },
    /**
     * 搜索关键字发生改变
    ...
    handleSearchKeywordChange(columnIndex) {
      ...
      function simulationClickItem() {
        for (let i = columnIndex + 1; i < this.data.length; i++) { this.$set(this.data, i, []) }
        if (columnIndex < this.maxLevel - 1 && keyword) {
          const currentItem = this.data[columnIndex].find(item => !item.$hidden)
          const currentIndex = this.data[columnIndex].findIndex(item => !item.$hidden)
          currentItem && currentIndex > -1 && this.handleClickItem(currentItem, currentIndex, columnIndex)
        } else if (columnIndex < this.maxLevel - 1 && !keyword) {
          this.handleClickItem(this.data[columnIndex][0], 0, columnIndex)
        }
      }

      if (!this.$$timer) {
        this.fn = this.debounce(simulationClickItem.bind(this), 300)
      }
      this.fn && this.fn()
    },
    /**
