<template>
  <div id="apply-index">
    <div class="entry-container">
      ...
      <!--店铺申请流程未完成-->
      <template v-if="shop_status === 'APPLY' || shop_status === 'WAIN_IMPROVED'">
      ...
      <template v-else-if="shop_status === 'APPLYING'">
        <h3>您的店铺申请流程尚未完成，无法对商家中心功能进行操作，您可以：</h3>
        <div class="apply-btn">
          <router-link to="/shop/apply/user-agreement">继续申请</router-link>
      ...
      <template v-else-if="shop_status === 'REFUSED'">
        <h3>抱歉您的申请开店
          <el-tooltip effect="dark" :content="audit_remark" placement="top-start">
            <span style="color: red">被拒绝</span>
          </el-tooltip>，无法对商家中心功能进行操作，您可以：</h3>
        <div class="apply-btn">
          <router-link to="/shop/apply/user-agreement">再次申请</router-link>
      ...
      <template v-else>
        ...
        <div class="apply-btn">
          <router-link to="/shop/apply/user-agreement">马上开店</router-link>
  ...
  mounted() {
    API_Shop.getApplyShopInfo().then(response => {
      if (response) {
        ...
        this.audit_remark = response.audit_remark
      }
    })
  }
}
