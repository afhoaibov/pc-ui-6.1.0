  ...
  <div class="category-manage">
    ...
    <el-dialog :title="categorytitle" :visible.sync="goodsCategoryShow" width="25%">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :loading="loadingbtn" @click="reserveCategoryGoods('goodsCatData')">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
    ...
    methods: {
      ...
      add_expanded(data) {
        if (Array.isArray(data) && data.length > 0) {
          data.forEach(elem => {
            this.$set(elem, 'expanded', this.goodsCatData.category_parent ? !(elem.shop_cat_id === this.goodsCatData.category_parent) : false)
      ...
      GET_GoodsCategoryList() {
        this.loading = true
        return API_goodsCategory.getGoodsCategoryList().then(response => {
          this.loading = false
      ...
      handleDeleteGoodsCategory(row) {
        this.$confirm('确认删除此分组, 是否继续?', '提示', { type: 'warning' }).then(() => {
          API_goodsCategory.deleteGoodsCategory(row.shop_cat_id).then(() => {
            this.GET_GoodsCategoryList()
      ...
      handleEditGoodsCategory($index, row) {
        ...
        const _shop_cat_pid = Number(row.shop_cat_pid) === 0 ? '' : row.shop_cat_pid
        this.goodsCatData = {
      ...
      reserveCategoryGoods(FormName) {
        ...
        if (current - this.lastTime > this.wait) {
            ...
            if (valid) {
              ...
              if (this.is_edit === 1) { // 添加
                ...
                API_goodsCategory.addGoodsCategory(params).then(() => {
                  ...
                  this.GET_GoodsCategoryList().then(res => {
                    this.$refs['tableLayout'].$refs['table'].data.forEach(row => {
                      const flag = this.goodsCatData.category_parent === row.shop_cat_id
                      this.$refs['tableLayout'].$refs['table'].toggleRowExpansion(row, flag)
                    })
                  })
                })
              } else { // 编辑
...
<style type="text/scss" lang="scss" scoped>
  .category-manage ::v-deep .el-table__body, .el-table__header, .el-table__footer {
    width: 100%;
    table-layout: fixed !important;
  }
  .category-manage ::v-deep div.toolbar {
    height: 70px;
    padding: 20px 0;
  }

  .category-manage ::v-deep .el-table td:not(.is-left) {
    text-align: center;
    width: 100%;
  }
  ...
  /*扩展样式修正*/
  .category-manage ::v-deep .el-table__expanded-cell {
     padding: 0;
  }
