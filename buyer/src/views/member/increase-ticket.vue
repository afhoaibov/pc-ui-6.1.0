  ...
  <div id="increase-ticket">
    <div class="member-nav">
      <ul class="member-nav-list">
        <li class="active">
          <router-link to="/member/increase-ticket" style="color:#f42424;">增票资质</router-link>
        </li>
      </ul>
    </div>
    <div class="ticket-container">
      ...
      <div class="ticket-information" v-if="isEditorInvoice">
        ...
        <el-form
          ...
          <el-form-item label="注册电话：" prop="register_tel">
            <el-input v-model="informationForm.register_tel" placeholder="请输入固定电话，例如：010-8888888" :maxlength="13"/>
          </el-form-item>
          ...
          <el-form-item class="agreed-form">
            <el-checkbox v-if="!informationFormExist" v-model="agreed" class="agreed">我已阅读并同意
              <el-button type="text" @click="openConfirm">《{{ articlesData.article_name }}》</el-button>
            </el-checkbox>
          </el-form-item>
          <el-form-item>
      ...
      <div class="ticket-information-exhibition" v-if="!isEditorInvoice">
        ...
        <ul>
          ...
          <li>纳税人识别号：
            <clipboard :text="informationForm.taxpayer_code" />
          </li>
          <li>注册地址：{{ informationForm.register_address }}</li>
          <li>注册电话：
            <clipboard :text="informationForm.register_tel" />
          </li>
          <li>开户银行：{{ informationForm.bank_name }}</li>
          <li>银行账号：
            <clipboard :text="informationForm.bank_account" />
          </li>
        </ul>
      </div>
      <div class="ticket-adress" v-if="informationFormExist">
        <template v-if="!adressFormObj">
          <h4>增票收票地址</h4>
          <div>您还未设置收票地址
            <el-button size="mini" style="margin-left:10px;" @click="isShow = true">立即设置</el-button>
          </div>
        </template>
        <template v-if="adressFormObj && isShow">
          <h4>增票收票地址
            <el-button v-if="!isShow" size="mini" type="text" @click="editTicketAdress">修改</el-button>
          </h4>
        </template>
        <el-form
          v-if="isShow"
          ...
          <el-form-item label="收票人手机号：" prop="member_mobile" class="mobile-item">
            <area-select ref="AreaSelect" :showType="1" v-model="adressForm.member_mobile"  />
          </el-form-item>
          ...
          <el-form-item label="详细地址：" prop="detail_addr">
            <el-input v-model="adressForm.detail_addr" :maxlength="50"/>
          </el-form-item>
          <el-form-item>
      ...
      <div class="ticket-information-exhibition" v-if="!isShow && adressFormObj && informationFormExist">
        <h4>增票收票地址
          <el-button size="mini" type="text" @click="editTicketAdress">修改</el-button>
        </h4>
        <ul>
          <li>收票人姓名：{{ adressFormObj.member_name }}</li>
          <li>收票人手机号：
            <clipboard :text="adressFormObj.member_mobile | formatMobile" />
          </li>
          <li>
            收票人省份：{{ adressFormObj.province }}{{ adressFormObj.city }}{{ adressFormObj.county }}{{ adressFormObj.town }}
          </li>
...
export default {
  ...
  data() {
    return {
      ...
      authInfoRules: {
        company_name: [
          this.MixinRequired('请输入公司名称'),
          {
            validator: (rule, value, callback) => {
              if (!RegExp.commonText.test(value)) {
                callback(new Error('只支持汉字、字母、数字的组合！'))
              } else {
                callback()
              }
            }
        ...
        region: [this.MixinRequired('请选择收票人地区')],
  ...
  mounted() {
    ...
    this.getArticlesByPosition()
  },
  methods: {
    getArticlesByPosition() {
      API_Article.getArticlesByPosition('INVOICE_CONFIRMATION_LETTER').then(res => {
        this.articlesData = res
      })
    },
    /** 增票资质确认书 */
    openConfirm() {
      this.$alert(this.articlesData.content, () => {
      }, {
        dangerouslyUseHTMLString: true,
        title: this.articlesData.article_name,
        area: '50%'
      })
    },
    /** 删除增票资质 */
    ...
    ticketConfirm() {
      this.$refs.informationForm.validate((valid) => {
        ...
        if (this.informationFormExist && this.isRefshExamine) {
          API_Members.changeTicketInformation({
            ...this.informationFormExist, ...this.informationForm,
            status: 'NEW_APPLY'
          }).then(res => {
            this.queryTicketInformation()
    ...
    submitAdressForm() {
      this.$refs.adressForm.validate((valid) => {
        ...
        const params = cloneObj(this.adressForm)
        params.member_mobile = this.$refs.AreaSelect.mobileValue
    ...
    /** 修改收票人地址 */
    editTicketAdress() {
      const adressFormObj = cloneObj(this.adressFormObj)

      this.adressForm = {
        id: adressFormObj.id || '',
        member_name: adressFormObj.member_name,
        member_mobile: adressFormObj.member_mobile,
        detail_addr: adressFormObj.detail_addr,
        country_name: adressFormObj.country_name,
        country_code: adressFormObj.country_code,
        province: adressFormObj.province,
        city: adressFormObj.city
      }
      this.defaultRegion = this.MixinGetRegionDefaultData(adressFormObj)
      this.isShow = true
    },
    queryTicketAdress() {
      API_Members.queryTicketAdress().then(res => {
        if (res) {
          this.adressFormObj = res
        }
      })
    },
    handleRegionChanged(obj) {
      this.$set(this.adressForm, 'region', obj)
      this.$refs.adressForm.validateField('region')
    }
  }
}
</script>
