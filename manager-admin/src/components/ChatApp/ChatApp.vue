  ...
  <div class="chat-app" @click="dismissAnyShow">
    <div class="chat-container">
      <div class="left-chat">
        <div class="user-info">
          <div class="face-user">
            <div class="img-face-user" :style="{backgroundImage: `url(${platformFace})`}"></div>
            <div class="info-user">
          ...
          <div class="chat-right">
            ...
            <div class="input-chat">
              <div class="toolbar-input-chat">
                ...
                <div v-if="play_audio === 0" class="btn-toolbar audio-text" @click="handleClickTool('audio-text')">
                  开启声音提醒
                </div>
          ...
          <div class="actions-right">
            ...
            <div class="content-actions">
              <div v-show="current_tab === 0" class="history-actions">
                ...
                <div v-if="currentAskGoods" class="tab-actions active">会员浏览历史</div>
                ...
                <div
                  class="tip-load-more"
                  :class="{disabled: goodsListParams.loading || goodsListParams.no_data || !receiver_id}"
...
export default {
  ...
  data() {
    return {
      ...
      play_audio: Storage.getItem('admin_play_audio'),
  ...
  async created() {
    const { member_id, goods_id } = this.$route.query
    this.receiver_id = member_id
  ...
  methods: {
    ...
    handleClickOrder(item) {
      ...
      let url = this.$router.resolve(`/order/detail/${order_sn || sn}`).href
      window.open(url)
    },
    /**
     * 加载更多商品
      ...
      this.$msgbox.confirm(
        '因浏览器限制，如需要使用声音提示，每次进入聊天界面，您需要点击下面确认按钮授权使用系统声音。\n如您不需要使用声音提醒，则点击下方的“不提示声音”按钮，本提示将不在弹出。',
        {
          ...
          callback: (res) => {
            if (res === 'cancel') {
              this.play_audio = 0
              Storage.setItem('admin_play_audio', 0)
            } else {
              this.play_audio = 1
              Storage.setItem('admin_play_audio', 1)
            }
          }
        }
      )
