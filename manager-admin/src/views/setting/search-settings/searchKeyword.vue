<template>
	<div class="container">
      ...
      <div class="table-tips-wrap" slot="tips">
        <div class="tips-box">
          <ul>
            <li style="margin-bottom: 10px;">为了搜索引擎分词（关键词）更加准确，可手工对拆词进行维护。</li>
            <li style="margin-bottom: 10px;">关键词添加后，索引不会自动生成；原有商品变更（修改）、新增商品会按照新的词库进行生成索引或者在管理端重新生成索引。</li>
          </ul>
        </div>
      </div>
			<div slot="toolbar" class="inner-toolbar">
				<div class="toolbar-btns">
					...
					<el-button size="mini" type="primary" @click="handleSetSecretKey">设置密钥</el-button>
				</div>
      ...
      :close-on-press-escape="false"
			...
			<el-form>
				...
				<el-form-item v-if="secretedKey" label="加密后" label-width="100px">
					<span>{{ secretedKey }}</span>
				</el-form-item>
			</el-form>
			<span slot="footer" class="dialog-footer">
  ...
  export default {
    ...
    watch: {
      $route: {
        immediate: true,
        handler(newVal) {
          if (newVal.name !== 'searchKeyword') return
    ...
    methods: {
      ...
      handleDelKeyword(index, row) {
        this.$confirm('确定要删除这个分词么？', '提示', { type: 'warning' }).then(() => {
          this.loading = true
      ...
      /** 设置密钥 */
      handleSetSecretKey() {
        this.dialogSecret = true
        API_search.getSecretKey().then(response => {
          this.loading = false
          this.secretKey = response.password
          this.secretedKey = response.secret_key
        }).catch(() => (this.loading = false))
      },
      ...
      setSecretKey() {
        ...
        API_search.setSecretKey({ password }).then(_ => {
          this.loading = false
