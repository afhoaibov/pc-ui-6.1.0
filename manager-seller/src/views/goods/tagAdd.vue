        ...
        <el-table-column label="商品信息" min-width="200">
          <template slot-scope="scope">
            <div class="goods-info">
              <a class="goods-name" :href="isO2O ? 'javascript:void(0)' : `${MixinBuyerDomain}/goods/${scope.row.goods_id}`">
                <img v-if="scope.row.thumbnail" :src="scope.row.thumbnail" class="goods-image"/>
              </a>
              <div class="goodsinfo-txt">
                <a class="goods-name" :href="isO2O ? 'javascript:void(0)' : `${MixinBuyerDomain}/goods/${scope.row.goods_id}`">{{ scope.row.goods_name }}</a>
                <span class="goods-price" v-if="!scope.row.exchange_point">{{ scope.row.price | unitPrice }}</span>
                <div class="goods-price" v-else>
                  <span v-if="scope.row.price && scope.row.price !== 0">
                    <i>{{ scope.row.price | unitPrice }}</i> + {{ scope.row.exchange_point }}积分
                  </span>
  ...
  export default {
    ...
    data() {
      return {
        ...
        /** 商品选择器列表api*/
        goodsApi: 'seller/goods?disabled=1',
    ...
    watch: {
      ...
      $route: {
        immediate: true,
        handler(newVal) {
          if (newVal.name !== 'tagAdd') return
          this.params.tag_id = this.tag_id = newVal.params.tag_id
          this.tag_name = newVal.query.tag_name
        }
      }
    },
    methods: {
      ...
      GET_TagGoodsList() {
        this.loading = true
        API_goodsTag.getTagGoodsList(this.params.tag_id, this.params).then(response => {
          this.loading = false
