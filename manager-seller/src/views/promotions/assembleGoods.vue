<template>
  <div class="sign-up-bg" v-loading="loading">
      ...
      <el-form-item label="活动商品:">
        <en-table-layout
          ...
          <template slot="table-columns">
            ...
            <el-table-column label="商品信息(SKU)" min-width="200" class="goods-sku-header">
              <template slot-scope="scope">
                <div class="goods-info">
                  <img :src="scope.row.thumbnail" alt="商品图片">
                  <div>
                    <a :href="isO2O ? 'javascript:void(0)' : `${MixinBuyerDomain}/goods/${scope.row.goods_id}?sku_id=${scope.row.sku_id}`" :target="isO2O ? '' : '_blank'" style="color: #00a2d4;"><span>{{ scope.row.goods_name }}</span></a>
                    <div class="sku-list">
                      <span>{{ scope.row.properties_name }}</span>
                    </div>
                  </div>
                </div>
              </template>
            ...
            <el-table-column label="库存(件)" prop="enable_quantity"/>
    ...
    <div style="text-align: center;">
      <el-button type="primary" @click="handleSaveAssembleGoods" v-if="activityBaseInfo.status !== 'END'">保存设置</el-button>
    </div>
    <en-goods-picker
      type="seller"
      sku
...
<script>
  ...
  export default {
    ...
    beforeRouteEnter(to, from, next) {
      next(vm => {
        vm.activityID = to.params.promotion_id
        vm.activityBaseInfo = { ...to.query }
        vm.GET_assembleGoodsDetails()
      })
    },
    ...
    computed: {
      canLookQrcode() {
        ...
        return current >= parseInt(this.activityBaseInfo.start_time) && current <= parseInt(this.activityBaseInfo.end_time)
      }
    },
    methods: {
      /** 获取活动商品详情 */
      ...
      handleSaveAssembleGoods() {
        ...
        const _params = this.activityData.goods_data.map(key => {
          return {
            ...
            specs: key.properties_name,
            origin_price: key.price
          }
        })
        API_activity.saveAssembleGoods(this.activityID, _params).then(() => {
          this.$message.success('保存成功')
          this.$router.push({ path: '/promotions/assembleManager' })
          this.$store.state.tagsView.cachedViews = this.$store.state.tagsView.cachedViews.filter(tab => tab !== 'assembleGoods')
          this.$store.state.tagsView.visitedViews = this.$store.state.tagsView.visitedViews.filter(tab => tab.name !== 'assembleGoods')
        })
      }
    }
  }
