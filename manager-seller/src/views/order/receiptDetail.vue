<template>
  <div class="detail-content">
    ...
    <div class="order-information">
      ...
      <p>
        订单编号：
        <clipboard :text="receiptDetail.order_sn" />
      </p>
      <p>订单金额：{{ receiptDetail.order_price | unitPrice }}</p>
    </div>
    <div style="border-top: 1px solid #ebeef5;">
      <en-table-layout :tableData="receiptDetail.order_sku_list" :toolbar="false" :pagination="false"
        :border-right="false">
        <template slot="table-columns">
          <el-table-column label="图片" width="90">
            <template slot-scope="scope">
              ...
              <img v-else :src="scope.row.goods_image" alt="" width="62" height="62">
            </template>
          </el-table-column>
          <el-table-column label="商品名称">
            <template slot-scope="scope">
              <a v-if="!MixinIsSupplier()" :href="isO2O ? 'javascript:void(0)' : `${MixinBuyerDomain}/goods/${scope.row.goods_id}`"
                :target="isO2O ? '' : '_blank'" class="goods-name">
    ...
    <div class="receipt-information">
      ...
      <template v-if="receiptDetail.receipt_type === 'ELECTRO'">
        <ul>
          ...
          <li v-if="receiptDetail.tax_no">
            发票税号：
            <clipboard :text="receiptDetail.tax_no " />
          </li>
          <li>
            收票人手机号：
            <clipboard :text="receiptDetail.member_mobile | formatMobile" />
          </li>
          ...
          <li v-if="receiptDetail.status === 1">电子发票：<a style="cursor: pointer;"
              :href="this.receiptDetail.elec_file_list[0]">点击下载</a></li>
          <li v-if="receiptDetail.status === 0 && !MixinIsSupplier()">
            <el-upload class="upload-demo" list-type="picture-card" :action="MixinUploadApi" :headers="MixinAccessToken"
              :on-success="handleSuccess" :before-upload="handleBeforeUpload"
              :on-exceed="() => { $message.error('只允许上传一个文件') }" :on-error="() => { $message.error('文件上传出错') }"
              :limit="1">
              <el-button type="primary">上传电子发票</el-button>
            </el-upload>
          </li>
          <li style="text-align: center;" v-if="receiptDetail.status === 0 && !MixinIsSupplier()">
      ...
      <template v-if="receiptDetail.receipt_type === 'VATORDINARY'">
        <ul>
          ...
          <li>收票地址：{{ `${receiptDetail.province || ''}${receiptDetail.city || ''}${receiptDetail.county ||
            ''}${receiptDetail.town || ''}${receiptDetail.detail_addr || ''}` }}</li>
          <li>开票状态：
            <span v-if="receiptDetail.status === 0" style="color:#f42424;">未开票</span>
            <span v-if="receiptDetail.status === 1" style="color:#67c23a;">已开票</span>
          </li>
          ...
          <li v-if="receiptDetail.status === 1">
            快递单号：
            <span>
            <clipboard :text="receiptDetail.logi_code" />
            </span>
          </li>
      ...
      <template v-if="receiptDetail.receipt_type === 'VATOSPECIAL'">
        <ul>
          ...
          <li>
            注册电话：
            <clipboard :text="receiptDetail.reg_tel" />
          </li>
          <li>开户银行：{{ receiptDetail.bank_name }}</li>
          <li>
            银行账户：
            <clipboard :text="receiptDetail.bank_account" />
          </li>
          <li>收票人姓名：{{ receiptDetail.member_name }}</li>
          <li>
            收票人手机号：
    ...
    <!--物流弹框-->
    <el-dialog title="填写快递单号" width="30%" :visible.sync="dialogVisible">
      <el-form ref="expressForm" :model="expressForm" label-width="80px">
        <el-form-item label="快递公司">
          <el-select v-model="expressForm.logi_id" placeholder="请选择快递公司">
            <el-option v-for="item in expressList" :key="item.logistics_company_do.id"
              :label="item.logistics_company_do.name" :value="item.logistics_company_do.id">
            </el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog :visible.sync="logisticsShow" width="400px" align="center">
      <div slot="title">
        ...
        <div class="logistics-base">
          <span>
            物流公司：
            <clipboard :text="receiptDetail.logi_name " />
          </span>
          <span>
            快递单号：
            <clipboard :text="receiptDetail.logi_code " />
      ...
      <div class="logistics-info">
        <div v-if="logisticsInfoList">
          <el-steps direction="vertical" :active="1" align-center space="100px">
            <el-step v-for="(row, index) in logisticsInfoList.data" :title="row.time" :key="index"
...
export default {
  ...
  watch: {
    $route: {
      immediate: true,
      handler(newVal) {
        if (newVal.name !== 'receiptDetail') return
        this.history_id = newVal.params.history_id
...
.detail-content ::v-deep .el-dialog__body {
  padding: 20px 25px;
