    ...
    <div class="group-buy w">
      ...
      <ul v-else class="group-buy-list">
        <li v-for="(gb, index) in groupBuy.data" :key="index">
          <div class="gl-item">
            <div class="gl-img">
              <router-link :to="'/goods/' + gb.goods_id + '?sku_id=' + gb.sku_id" target="_blank">
                <img :src="gb.img_url" :alt="gb.goods_name">
              </router-link>
            </div>
            <div class="gl-price">
              <strong>
                <en-price :price="gb.price" direction="row" size="20" />
              </strong>
              <span class="gl-original-price">
                <en-price :price="gb.original_price" lineThrough direction="row" size="20" />
              </span>
            </div>
            <div class="gl-name">
              <router-link :to="'/goods/' + gb.goods_id + '?sku_id=' + gb.sku_id" :title="gb.goods_name" target="_blank" v-html="gb.goods_name">
                <em>{{ gb.goods_name }}</em>
              </router-link>
            </div>
            <p class="gl-sub-title">{{ gb.gb_title }}</p>
            <div class="gl-buy-count">已销售：{{ gb.show_buy_num }}件</div>
            <router-link :to="'/goods/' + gb.goods_id + '?sku_id=' + gb.sku_id" target="_blank" class="gl-join-btn">我要参团</router-link>
          </div>
        </li>
      </ul>
      <el-pagination
...
#group-buy ::v-deep .el-pagination {
