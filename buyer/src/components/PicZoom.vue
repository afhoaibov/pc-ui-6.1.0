...
-->
<template>
  <div class="magnifier-box" :class="[vertical && 'vertical']" :ref="id" @mousemove.prevent="mousemove"
...
export default {
  ...
  methods: {
    ...
    initTime() {
      ...
      this.img.src = imgsrc
      this.img.onload = () => {
        this.rectTimesX = this.imgbox.offsetWidth / this.scale / this.imgwrap.offsetWidth // 0.4
        this.rectTimesY = this.imgbox.offsetHeight / this.scale / this.imgwrap.offsetHeight // 0.4
    ...
    mousemove(e) {
      ...
      let startX = pos.x - (imgwrap.left - (document.documentElement.scrollLeft || document.body.scrollLeft))
      let startY = pos.y - (imgwrap.top - (document.documentElement.scrollTop || document.body.scrollTop))
      this.ctx.drawImage(
        this.img,
