  ...
  export default {
    ...
    watch: {
      $route: {
        immediate: true,
        handler(newVal) {
          if (newVal.name !== 'addTimeLimit') return
    ...
    methods: {
      ...
      GET_limitTimeActivityDetails() {
        API_activity.getLimitTimeActivityDetails(this.activityID).then(response => {
          this.loading = false
      ...
      handleSignUp() {
        ...
        for (let i = 0; i < _params.length; i++) {
          let _price = _params[i].price
          if (!RegExp.money.test(_price) || _price <= 0) {
        ...
        this.$confirm('参与此活动进行报名的商品仅可在此次报名操作中编辑, 是否继续,请谨慎操作?', '提示', { type: 'warning' }).then(() => {
          API_activity.signUpLimitTimeActivity(activityID, _params).then(() => {
