...
async function routerBeforeEach(to, from, next) {
  ...
  if (refreshToken) {
    if (to.path === '/login') {
      ...
      if (status === 'OPEN') {
        ...
        MessageBox.confirm('账号尚未申请开店！', '出现错误', {
          ...
          location.replace(`${Mixin.computed.MixinDomain().buyer_pc}/shop/apply`)
        })
      }
    }
  } else {
    if (whiteList.indexOf(to.path) !== -1) {
      ...
      next(`/login?forward=${to.fullPath}`)
      NProgress.done()
    }
  }
}
...
const errorMsg = (msg, title = '权限错误') => {
  ...
  MessageBox.alert(msg, title, {
    type: 'error',
    customClass: 'shop-status-alert',
    callback: () => {
      location.replace(router.resolve('/login').href)
    }
  }).then(() => {
    // 强制设置弹窗的 z-index
    setTimeout(() => {
      const messageBox = document.querySelector('.shop-status-alert')
      if (messageBox) {
        messageBox.style.zIndex = '999999'
        const wrapper = messageBox.querySelector('.el-message-box__wrapper')
        if (wrapper) {
