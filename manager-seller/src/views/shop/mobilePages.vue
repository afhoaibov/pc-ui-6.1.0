    ...
    >
      ...
      <div slot="toolbar" class="inner-toolbar">
        <div class="toolbar-btns">
          ...
          <el-button size="mini" type="primary" :disabled="selected.length !== 1" @click="handleSetIndex">设为店铺首页</el-button>
...
export default {
  ...
  computed: {
    /** 装修服务地址 */
    decor_url() {
      let url = this.MixinDomain.decor + '?decor_type=seller&client_type=mobile'
  ...
  methods: {
    ...
    async handleCopyPage(row) {
      ...
      API_Pages.copyMiniPages(id).then(() => {
    ...
    async handleDeletePage(row) {
      ...
      API_Pages.deleteMiniPages(id).then(() => {
    ...
    GET_MiniPages() {
      this.loading = true
      API_Pages.getMiniPages(this.params).then(response => {
    ...
    async handleChangePublishStatus(row) {
      await API_Pages.changeMiniPagePublishStatus(row.id)
      this.GET_MiniPages()
    },
    ...
    async handleSetIndex(row) {
      if (!row || !row.id) {
        ...
        if (selected[0].is_index === 'YES') return this.$message.error('该页面已经是店铺首页了！')
        row = selected[0]
      }
      try {
        if (row['is_index'] === 'NO') {
          await this.$confirm('要将这个页面设置为店铺首页吗？')
        }
        await API_Pages.setMiniPageIndex(row.id)
        this.GET_MiniPages()
      } catch (e) {
        //
      }
    ...
    getMiniPageLink(page) {
      let url = this.MixinDomain['buyer_wap']
      if (page.is_index === 'YES') {
        url += `/pages/shop/shop?shop_id=${page.seller_id}`
      } else {
        url += `/pages/mini-page/mini-page?id=${page.id}`
      }
      return url
    }
  }
