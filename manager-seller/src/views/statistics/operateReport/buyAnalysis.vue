<template>
  <div class="bg-shop-summary">
    ...
    <div id="priceList" :style="{height: tableHeight + 'px', marginTop: '30px'}"></div>
    <div id="purchaseTime" :style="{height: tableHeight + 'px', marginTop: '30px'}"></div>
    ...
    methods: {
      ...
      countTableHeight() {
        ...
        if (this.priceListChart) {
          this.priceListChart.resize()
        }
        if (this.purchaseTimeChart) {
          this.purchaseTimeChart.resize()
        }
      },
      ...
      GET_PriceStatistics() {
        API_buyAnyalysis.getbuyAnyalysisPeriodList(this.params).then(response => {
          this.purchaseTimeChart = echarts.init(document.getElementById('purchaseTime'))
          this.purchaseTimeChart.setOption({
            title: {
              text: '购买时段分布',
              x: 'center',
              subtext: '当前时间段24小时购买总量分布',
              top: 0
            },
        ...
        API_buyAnyalysis.getbuyAnyalysisRangesList(this.params).then(response => {
          this.priceListChart = echarts.init(document.getElementById('priceList'))
          this.priceListChart.setOption({
            title: { text: '客单价', x: 'center' },
            tooltip: { trigger: 'axis' },
