<template>
  <div id="shop-list">
    ...
    <div class="shop-list-container w">
      ...
      <div v-if="!shopList || !shopList.data.length" style="text-align:center;line-height:200px">
        ...
        <li v-for="shop in shopList.data" :key="shop.shop_id" class="shop-item">
          <div class="shop-info">
            <div class="shop-member-face">
              <router-link :to="'/shop/' + shop.shop_id" class="shop-name">
          ...
          <div :class="['shop-goods', shop.goods_on && 'on']">
            <template v-if="!shop.goods_list || !shop.goods_list.length">
              <p class="no-goods">哎呀！店铺还没商品呢，去
                <router-link :to="'/shop/' + shop.shop_id">店铺首页</router-link>
                看看试试吧！
              </p>
            </template>
            <ul v-else>
              <template v-for="(goods, index) in shop.goods_list">
                <li v-if="index < 5" :key="goods.goods_id" class="goods-item">
                  <router-link :to="'/goods/' + goods.goods_id" target="_blank">
                    ...
                    <p class="goods-price price">
                      <!-- {{ goods.price | unitPrice }} -->
                      <en-price :price="goods.price || 0" :sizeRatio="0.8" size="12" direction="row" />
                    </p>
                    <p class="goods-name" v-html="goods.goods_name">{{ goods.goods_name }}</p>
                  </router-link>
                </li>
              </template>
            </ul>
...
import ShopStar from './-shop/-shop-star'

export default {
  name: 'shopList',
  components: { ShopStar, 'el-pagination': Pagination },
  head() {
    return {
      title: `店铺列表-${this.site.title}`
    }
  },
  data() {
    return {
      ...
      params: {
        page_no: 1,
        page_size: 20,
        shop_name: this.$route.query.keyword
      },
      loading: true
    }
  },
  watch: {
    '$route.query': function(newName, oldName) {
      if (newName.keyword !== oldName.keyword) {
        this.params = {
          page_no: 1,
          page_size: 20,
          shop_name: newName.keyword
        }
        this.GET_ShopList()
      }
    }
  ...
  methods: {
    ...
    formatAddress(shop) {
      ...
      let address = `${shop.shop_province} ${shop.shop_city} ${shop.shop_county}`
      if (shop.shop_town) {
        address += ` ${shop.shop_town}`
      }
      return address
    }
  }
...
.shop-list-container {
  ...
  .shop-other-info {
    ...
    .shop-score-arrow {
      ...
      height: 0;
      border-width: 4px;
      border-color: theme.$color-main transparent transparent transparent;
...
.shop-list-container ::v-deep .el-pagination {
  text-align: right;
  margin-bottom: 20px;
}
</style>
