...
<script>
  ...
  // 涉及导出相关页面权限
  const exportIdentifier = ['goodsList', 'putforwardApply', 'collectionList', 'orderList', 'refundList', 'serviceList', 'settlement']
  // 涉及翻译任务页面权限
  const translationIdentifier = ['dictionaryManage', 'translationTask']
  export default {
    name: 'rolePermission',
    data() {
      return {
    ...
    watch: {
      ...
      $route: {
        immediate: true,
        handler(newVal) {
          if (newVal.name !== 'rolePermission') return
          this.role_id = newVal.params.id
    ...
    methods: {
      ...
      /** 设置导出记录是否选中 */
      setExportChecked(item) {
        // 勾选了涉及导出相关页面时  默认勾选后台任务 > 导出记录权限
        if (item.checked && this.exportIdentifier.some(v => v === item.identifier)) {
          this.permissions.map((permissionsItem) => {
            if (permissionsItem.identifier === 'operations') {
              permissionsItem.children.map(child => {
                if (child.identifier === 'backstageTask') {
        ...
        if (item.checked && this.translationIdentifier.some(v => v === item.identifier)) {
          this.permissions.map((permissionsItem) => {
            if (permissionsItem.identifier === 'operations') {
              permissionsItem.children.map(child => {
                if (child.identifier === 'backstageTask') {
                  const roleChild = child.children.find(v => v.identifier === 'translationRecord')
                  roleChild.checked = true
                }
              })
            }
          })
