    ...
    >
      <el-form :model="shopThemeForm" :rules="shopThemeRules" ref="shopThemeForm" label-width="100px">
        ...
        <el-form-item label="模板标识" prop="mark">
          <el-input v-model="shopThemeForm.mark" :maxlength="50" placeholder="确保唯一性，且不可与PC模板重复"></el-input>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      uploadSuccess(res) {
        this.$set(this.shopThemeForm, 'image_path', res.url)
        this.$refs['shopThemeForm'].validateField('image_path')
      },
      /** 图片移除之后 */
      delGiftImg() {
        this.fileList = []
        this.shopThemeForm.image_path = ''
      },
      /** 分页大小发生改变 */
      ...
      handleEditShopTheme(index, row) {
        ...
        this.fileList = this.shopThemeForm.image_path ? [{ name: '模板图片', url: this.shopThemeForm.image_path }] : []
        this.dialogShopThemeVisible = true
      },
