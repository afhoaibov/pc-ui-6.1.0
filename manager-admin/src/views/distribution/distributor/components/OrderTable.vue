  ...
  <div class="brokerage-wrap">
        ...
        <tbody v-for="(item, index) in tableData" :key="index">
          ...
          <tr>
            ...
            <td style="display: flex;justify-content: center;padding-top: 20px;">
              <div class="user-info" v-if="item.agent_member_id_lv1">
                <img v-if="item.agent_member_avatar_lv1" :src="item.agent_member_avatar_lv1" alt="">
                <img v-else src="@/assets/images/icon-noface.jpg" alt="">
                <p>{{item.agent_member_name_lv1}}</p>
              </div>
              <div class="user-info" v-if="item.agent_member_id_lv2">
                <img v-if="item.agent_member_avatar_lv2" :src="item.agent_member_avatar_lv2" alt="">
                <img v-else src="@/assets/images/icon-noface.jpg" alt="">
                <p>{{item.agent_member_name_lv2}}</p>
              </div>
            </td>
            <!-- 结算金额 -->
            <td>
            ...
            <td>
              <div style="display: flex;flex-direction: column;">
                <span style="color: green;">+{{item.commission_total | unitPrice}} </span>
                <span style="color: red;">-{{item.return_commission_total | unitPrice}} </span>
                <el-button class="theme-color" type="text" @click="handleCommissionDetail(item)">分佣详情</el-button>
              </div>
            </td>
            <!-- 佣金状态 -->
            <td>
    ...
    >
      <div style="margin-bottom: 20px;display: flex;justify-content: space-around;">
        <div style="font-size: 16px;font-weight: bold;">结算金额<p style="color: blue;">{{ dialogLine.goods_total_amount | unitPrice}}</p></div>
        <div style="font-size: 16px;font-weight: bold;">佣金总额<p style="color: green;">{{ dialogLine.commission_total | unitPrice}}</p></div>
        <div style="font-size: 16px;font-weight: bold;">退回佣金<p style="color: red;">{{ dialogLine.return_commission_total | unitPrice}}</p></div>
      </div>
      <table class="my-table">
        <thead>
          <tr>
        ...
        <tbody v-for="(item, index) in dialogTableData" :key="index">
          <td>
            <div class="user-info" style="display: flex;flex-direction: column;align-items: center;margin-top: 10px;">
          ...
          <td>
            <div>
              <span v-if="item.commission_level === 1">一级</span>
              <span v-if="item.commission_level === 2">二级</span>
            </div>
          </td>
          <td>
            <div>
  ...
  export default {
    ...
    methods: {
      ...
      handleCommissionDetail(item) {
        this.dialogLine = item
        API_distribution.getOrderCommissionDetail(item.order_sn, item.sku_id).then(response => {
          console.log(response)
          this.dialogTableData = response
      ...
      getDistributionOrderList() {
        ...
        API_distribution.getDistributionOrderList(params).then(response => {
