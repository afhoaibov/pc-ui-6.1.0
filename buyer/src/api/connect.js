...
export function getConnectUrl(client_type, login_type) {
  return `${process.env.API_BUYER || api.buyer}/buyer/connect/${login_type}/${client_type}/login`
}
...
export function loginByConnect(uuid, params) {
  ...
  return request({
    url: `/buyer/passport/login-binder/pc/${uuid}`,
    method: Method.PUT,
    data: params
  })
}
...
export function loginBindConnect(uuid) {
  return request({
    url: `/buyer/account-binder/login/${uuid}`,
    method: Method.POST,
    needToken: true
  })
}
...
export function registerBindConnect(uuid) {
  return request({
    url: `/buyer/account-binder/register/${uuid}`,
    method: Method.POST,
    needToken: true
  })
}
