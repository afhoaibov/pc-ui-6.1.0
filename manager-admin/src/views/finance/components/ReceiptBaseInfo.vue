<template>
  <div v-loading="loading" class="receipt-base-info">
    <el-row :gutter="0">
      <el-col :span="24">
        ...
        <div class="d-content">
          ...
          <div class="item">
            <span class="item-label">订单编号</span>
              <clipboard :text="receipt.order_sn" />
          </div>
          <div class="item">
        ...
        <div class="d-content">
          ...
          <div v-if="receipt.receipt_type === 'ELECTRO'">
            ...
            <div class="item">
              ...
              <span class="item-value">{{ receipt.member_mobile || '无' | secrecyMobile }}</span>
            </div>
            ...
            <div class="item" v-if="receipt.receipt_type === 'ELECTRO' && receipt.status === 1">
              ...
              <a style="cursor: pointer;color:#409EFF;font-size: 14px;" :href="this.receipt.elec_file_list[0]"
          ...
          <div v-if="receipt.receipt_type === 'VATORDINARY'">
            ...
            <div class="item">
              ...
              <span class="item-value">{{ `${receipt.province || ''}${receipt.city || ''}${receipt.county ||
                ''}${receipt.town || ''}${receipt.detail_addr || ''}` || '无' }}</span>
            </div>
            <div class="item">
              <span class="item-label">开票状态</span>
              <span v-if="receipt.status === 0" class="item-value">未开票</span>
            ...
            <div v-if="receipt.status === 1">
              ...
              <div class="item">
                ...
                <el-button type="text" @click="checkLogistics">点击查看</el-button>
              </div>
            </div>
          </div>
          <!--增值税专用发票-->
          <div v-if="receipt.receipt_type === 'VATOSPECIAL'">
            ...
            <div class="item">
              ...
              <clipboard v-if="receipt.tax_no" :text="receipt.tax_no" />
              <span class="item-value" v-else>无</span>
            </div>
            <div class="item">
            ...
            <div class="item">
              ...
              <clipboard v-if="receipt.reg_tel" :text="receipt.reg_tel" />
              <span class="item-value" v-else>无</span>
            </div>
            <div class="item">
            ...
            <div class="item">
              <span class="item-label">收票人手机号</span>
              <clipboard v-if="receipt.member_mobile" :text="receipt.member_mobile | secrecyMobile" />
              <span class="item-value" v-else>无</span>
            </div>
            <div class="item">
              <span class="item-label">收票地址</span>
    ...
    <!--查看物流信息弹框-->
    <el-dialog :visible.sync="logisticsShow" width="400px" :modal="false" align="center">
      <div slot="title">
...
export default {
  ...
  methods: {
    ...
    /** 获取开票详情 */
    GET_ReceiptDetail(id) {
      API_Receipt.getHistoryReceiptDetail(id).then(response => {
...
.receipt-base-info ::v-deep .el-dialog__body {
  padding: 20px 25px;
...
.receipt-base-info ::v-deep .el-table td:not(.is-left) {
  text-align: center;
}
