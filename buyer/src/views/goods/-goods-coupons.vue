<template>
  <div v-if="coupons && coupons.length" id="goods-coupons">
      ...
      <div class="pro-content coupons">
        <template v-for="(coupon, index) in coupons">
          ...
          >
            ...
            <div class="tj" sf-text-rule="coupon_tips">
              满
              <en-price
                :price="coupon.coupon_threshold_price"
                direction="row"
                :sizeRatio="0.9"
                size="12"
              />
    ...
    <div :class="['coupons-sidebar', sidebarOpen && 'open']" @click.stop>
      <div class="title-sidebar">
        <i class="icon-title-sidebar icon-coupons"></i>
        <span class="title-title-sidebar">优惠券</span>
        <i class="icon-title-sidebar icon-close" @click="closeSidebar"></i>
      </div>
        ...
        <div class="items-coupons">
          ...
          <div class="content-items-coupons">
            <div v-for="coupon in coupons" :key="coupon.coupon_id" class="item-b">
              <div class="detail-b">
                <div class="money-b">
                  <en-price :price="coupon.coupon_price" direction="row" size="24"/>
                </div>
                <div class="other-b">
                  <p>{{ coupon.title }}</p>
                  <p class="color-gary">
                  ...
                  <p class="color-gary" v-if="coupon.effective_type === 'AUTO_TAKE_EFFECT'">
                    有效期{{ coupon.start_time | unixToDate('yyyy-MM-dd') }} 至
                    {{ coupon.end_time |unixToDate('yyyy-MM-dd') }}
                  </p>
                  <p class="color-gary" v-if="coupon.effective_type === 'EFFECTIVE_AFTER_RECEIVING'">
                    领取后{{ coupon.effective_days }}天内有效
                  </p>
                </div>
              </div>
              <div class="btn-b">
  ...
  methods: {
    ...
    GET_CouponsList() {
      ...
      API_Promotions.getOwnCoupons(goods_id).then(response => {
        ...
        if (selectedSku && Object.keys(selectedSku).length && response.length) {
          const sku_coupons = this.couponsAll.find(key => key.sku_id === selectedSku.sku_id).coupon_list
