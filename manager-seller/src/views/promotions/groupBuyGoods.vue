    ...
    <el-form ref="gruopBuyForm" :model="gruopBuyForm" :rules="rules" label-width="120px" status-icon>
      ...
      <el-form-item label="团购名称" prop="gb_name">
        <el-input
          ...
          placeholder="团购名称长度最多可输入30个字符">
        </el-input>
        <span class="activity-tip">团购名称长度最多可输入30个字符</span>
      </el-form-item>
      <!--团购标题-->
      ...
      <el-form-item label="团购价格" prop="price">
        ...
        <span class="activity-tip">
          团购价格为该商品参加活动时的促销价格必须是0.01-100000000之间的数字
        </span>
      </el-form-item>
      <!--团购图片-->
      <el-form-item label="团购图片" prop="img_url">
        <el-upload
          ...
          <div slot="tip" class="el-upload__tip">请上传jpg、jpeg、png格式，建议大小不超1M的1:1正方形图片且宽高误差应在0～50之间</div>
        </el-upload>
      </el-form-item>
      <!--商品库存数-->
      ...
      <el-form-item label="虚拟数量" prop="visual_num">
        <el-input
          v-model="gruopBuyForm.visual_num" maxlength="9"
          :style="{ width:inputLength +'px' }"
  ...
  export default {
    ...
    data() {
      const checkGroupBuyPrice = (rule, value, callback) => {
        ...
        } else if (value < 0.01 || value > 100000000) {
          callback(new Error('团购价格须在0.01～100000000 之间'))
        } else {
          callback()
        }
      }
    ...
    watch: {
      $route: {
        ...
        handler(newVal) {
          ...
          if (goods_id) {
            this.GET_GroupBuyGoodsDetails(goods_id)
          }
          this.GET_AllGroupBuyActivitys()
          this.GET_GroupCateGories()
          this.$nextTick(() => {
            this.$refs.gruopBuyForm.resetFields()
          })
        }
      }
    },
    methods: {
      ...
      refreshFunc(val) {
        ...
        if (_val && _val.goods_name) {
          ...
          this.gruopBuyForm.sku_id = _val.sku_id
          ...
          this.gruopBuyForm.goods_num = _val.quantity
        }
      },
        ...
        this.$refs['gruopBuyForm'].validate((valid) => {
          if (valid) {
            if (this.$route.params.goods_id) { // 编辑
              this.gruopBuyForm.specs = JSON.stringify(this.gruopBuyForm.specs)
...
<style type="text/scss" lang="scss" scoped>
  ...
  /*协议*/
  .bg-group-buy ::v-deep .is-allow-agreement {
    .el-button--text{
      font-size: 14px;
    }
  }
