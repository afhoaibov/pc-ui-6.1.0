...
module.exports = {
  ...
  /**
   * o2o功能开关
   * 如果您需要开启o2o相关功能，请设置为true
   */
  o2o: false,
  /**
   * 供应商功能开关
   * 如果您需要开启供应商相关功能，请设置为true
   */
  supplyGood: false,
  /**
   * IM功能开关
...
function assetsUrl() {
  ...
  if (assets.seller) {
    url = assets.seller
    if (!assets.seller.endsWith('/')) url += '/'
  } else if (alias.seller) {
    if (!alias.seller.startsWith('/')) url += '/'
    url += alias.seller
    if (!alias.seller.endsWith('/')) url += '/'
  }
  url = url.replace(/\/\//g, '/')
  return url
}
