  ...
  >
    ...
    <div class="dorpdown-layer">
      <div class="header-cart-content">
        <div class="mc">
          <div v-if="shopList.length > 0" class="header-cart-list">
            <ul v-for="shop in shopList" :key="shop.shop_id" :class="[shop.invalid === 1 && 'invalid']">
              <li
                v-for="group in shop.group_list"
                :key="group.promotion_id"
              >
                <div v-for="sku in group.sku_list" :key="sku.sku_id"
                  class="item-header-cart"
                  :class="[sku.invalid === 1 && 'invalid']"
                >
                  <div class="p-img">
                    <router-link :to="'/goods/' + sku.goods_id" target="_blank" :title="sku.name">
                      <img :src="sku.goods_image" :alt="sku.name">
                    </router-link>
                  </div>
                  <div class="p-name">
                    <router-link :to="'/goods/' + sku.goods_id" target="_blank" v-html="sku.name">
                      {{ sku.name }}
                    </router-link>
                  </div>
                  <div class="p-number">
                  ...
                  <div class="p-oper">
                    <div class="price"><span class="item-total"><en-price :price="sku.subtotal"/></span></div>
                    <a @click="handleRemoveSkuItem(sku)" href="javascript:void(0);" class="remove">删除</a>
                  </div>
                </div>
              </li>
              <li
                v-for="sku in shop.sku_list"
                :key="sku.sku_id"
                class="item-header-cart"
                :class="[sku.invalid === 1 && 'invalid']"
              >
        ...
        <div class="mb">
          <div class="p-total">共<span class="cart-num">{{ allCount }}</span>件商品&nbsp;&nbsp;共计：
            <span class="cart-total"><en-price :price="cartTotal.goods_price || 0"/></span>
          </div>
          <router-link to="/cart" class="btn-cart">去购物车</router-link>
        </div>
      </div>
    </div>
  </div>
