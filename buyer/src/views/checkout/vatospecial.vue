  ...
  <li class="increment-receipt">
    ...
    <div class="increment-receipt-content">
      <div class="" v-if="step1 && !step2 && !step3">
        <el-form v-if="step1" :rules="authInfoRules" size="mini">
      ...
      <div class="" v-if="step2">
        <el-form v-if="step2" :model="companyInformationForm" ref="companyInformationForm" size="mini" label-width="130px"
          class="ticket-form">
      ...
      <!--收票人地址信息-->
      <el-form v-if="step3" :model="adressForm" :rules="authInfoRules" ref="adressForm" size="mini" label-width="130px"
        class="ticket-form">
        ...
        <el-form-item label="收票人手机号：" prop="member_mobile" class="mobile-item">
          <area-select
            ...
            ref="AreaSelect"
            :showType="1"
            v-model="adressForm.member_mobile" />
        </el-form-item>
        <el-form-item label="收票人所在地" prop="receipt_region">
          <en-region-picker ref="regionPickerRef" :api="MixinRegionApi" :default="defaultRegion" @changed="handleRegionChanged('receipt', $event)"/>
...
export default {
  name: 'vatospecial',
  props: ['receipt', 'tabData', 'orderWay'],
  ...
  data() {
    return {
      ...
      authInfoRules: {
        ...
        receipt_region: [this.MixinRequired('请选择收票人地址')],
        member_mobile: [
  ...
  async mounted() {
    ...
    if (_receipt.detail_addr) {
      this.adressForm = {
        ...this.adressForm,
        province: _receipt.province,
        country_name: _receipt.country_name,
        country_code: _receipt.country_code,
        city: _receipt.city,
        detail_addr: _receipt.detail_addr,
        member_mobile: _receipt.member_mobile,
        member_name: _receipt.member_name
      }
      this.defaultRegion = this.MixinGetRegionDefaultData(_receipt)
    }
  },
  methods: {
    /** 获取赠票资质详情信息 */
    queryTicketInformation() {
      let api = API_Members.queryTicketInformation()
    ...
    submit(callback) {
      this.$refs.adressForm.validate((valid) => {
        ...
        let { detail_addr, member_mobile, member_name } = this.adressForm
        if (areaValue_n && member_mobile.indexOf(',') === -1) {
          member_mobile = `${areaValue_n},${member_mobile}`
        }
        let params = {
          detail_addr,
          member_mobile,
    ...
    handleRegionChanged(type, object) {
      ...
      this.$set(this.adressForm, key, JSON.stringify(object))
      this.$refs.adressForm.validateField(key)
    }
  },
  beforeDestroy() {
