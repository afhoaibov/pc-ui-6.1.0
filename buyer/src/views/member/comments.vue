    ...
    <el-form v-if="!cantComments" :model="commentsForm" :rules="commentsRules" ref="commentsForm">
      ...
      <div class="comments-goods-box">
        <div v-for="(comment, index) in commentsForm.comments" :key="index" class="comments-item">
          <!-- 商品展示 -->
          <router-link v-if="isFirstComment" :to="'/goods/' + skulist[index].goods_id + '?sku_id=' + skulist[index].sku_id" target="_blank" class="comments-goods">
            <div class="goods-image"><img :src="skulist[index].goods_image" :alt="skulist[index].name"></div>
            <div class="goods-name">
              <span v-html="skulist[index].name">{{ skulist[index].name }}</span>
              <p class="sku-spec">{{ skulist[index] | formatterSkuSpec }}</p>
            </div>
          </router-link>
          <router-link v-else :to="'/goods/' + goodsCommentsInfo[index].goods_id + '?sku_id=' + goodsCommentsInfo[index].sku_id" target="_blank" class="comments-goods">
            <div class="goods-image">
              <img :src="goodsCommentsInfo[index].goods_img" :alt="goodsCommentsInfo[index].goods_name"></div>
            <div class="goods-name">
              <span>{{ goodsCommentsInfo[index].goods_name }}</span>
              <p class="sku-spec">{{ goodsCommentsInfo[index] | formatterSkuSpec }}</p>
            </div>
          </router-link>
          <!-- 评价 -->
          <div class="comments-content">
            ...
            <ul class="comments_score" v-else>
              <template v-if="comment.$$goods_comment">
                ...
                <li>
                  初评评分：{{ comment.$$goods_comment.grade === 'good' ? '好评' : comment.$$goods_comment.grade === 'neutral' ? '中评' : comment.$$goods_comment.grade === 'bad' ? '差评' : '' }}
                </li>
                <li>初评内容：{{ comment.$$goods_comment.content }}</li>
                <li v-if="comment.$$goods_comment.images">
                  <span style="float:left;">初评图片：</span>
                  <viewer :images="comment.$$goods_comment.images">
                    <a
                      :key="index"
                      v-for="(img, index) in comment.$$goods_comment.images"
                    >
                      <img class="comments-images" :src="img" alt="">
                    </a>
                  </viewer>
                </li>
                <li>
            ...
            <div :class="['comments-images', comment.images.length >= 5 && 'limit']" v-if="isFirstComment || comment.$$goods_comment.audit_status === 'PASS_AUDIT'">
              <el-upload
                ...
                :limit="5"
                :on-change="(file,fileList) => { comment.images = fileList }"
                :on-remove="(file, fileList) => { comment.images = fileList }"
              >
                <i class="el-icon-plus"></i>
                <div slot="tip" class="el-upload__tip">请上传jpg、jpeg、png格式，建议分辨率不要超过800*800、大小不超1M的的1:1正方形图片</div>
              </el-upload>
            </div>
            <p v-if="isFirstComment || comment.$$goods_comment.audit_status === 'PASS_AUDIT'">
              您收到的商品是什么样的呢？晒个照片吧（最多可上传5张照片）</p>
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      append_comment: this.$route.query.append_comment,
      skulist: '',
      commentsForm: {},
      commentsRules: {
        comments: []
  ...
  mounted() {
    let append_comment = this.append_comment
    ...
    API_Order.getOrderDetail(this.order_sn).then(async response => {
      ...
      const commentStatus = response.comment_status // 提取评论状态
      if (commentStatus !== 'UNFINISHED' && append_comment !== 'review') {
        this.isFirstComment = false
        this.cantComments = true
      }
      if (append_comment === 'review') {
        this.isFirstComment = false
        this.goodsCommentsInfo = commentResult = await API_Members.getGoodsFirstComments({
      ...
      this.commentsForm = {
        ...
        comments: commentResult.map((item, index) => {
          ...
          this.commentsRules.comments[index].content = [{
            required: false,
            message: '可以告诉我们您遇到了什么问题吗？',
            trigger: 'blur'
          }]
          return {
            $$goods_comment: item,
      ...
      this.$refs['commentsForm'].validate((valid) => {
        if (valid) {
          ...
          params.comments.forEach(key => {
            ...
            if (key.images.some(item => item.status !== 'success')) {
...
.comments-goods-box {
  ...
  .comments-input ::v-deep .el-form-item.is-success .el-textarea__inner {
    border-color: #dcdfe6;
  }
  .comments-images {
    width: 630px;
    ...
    ::v-deep .el-upload-list__item, .el-upload--picture-card, .el-progress {
      width: 100px;
      height: 100px;
    }
    ::v-deep .el-upload--picture-card, .el-progress {
      line-height: 100px;
    }
