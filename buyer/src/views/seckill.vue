          ...
          >
            <a href="javascript:;" class="main-timeline-item">
              <div class="timeline-wrap">
                ...
                <div class="next-timeline"><i>{{ parseInt(timeLine.distance_time) === 0 ? '正在抢购' : '即将开始' }}</i></div>
                <div class="time-timeline">
                  <b class="b-text">{{ parseInt(timeLine.distance_time) === 0 ? '正在抢购' : '即将开始' }}</b>
                  <b class="b-time">
                    {{ parseInt(timeLine.distance_time) === 0 ? (onlyOne ? '距结束' : '距下一轮') : '距开始' }}
                    <i>{{ timesText[index].hours }}</i>:<i>{{ timesText[index].minutes }}</i>:<i>{{ timesText[index].seconds }}</i>
                  </b>
                </div>
              </div>
            </a>
    ...
    <div class="seckill-goods w">
      <ul class="seckill-list">
        <li v-for="(goods, index) in goodsList.data" :key="index" class="goods-item">
          <router-link :to="'/goods/' + goods.goods_id + '?sku_id=' + goods.sku_id" class="link-goods-item">
          ...
          <div :class="['info-goods-item',countProgress(goods)===100 && 'end', !seckillIsStart && 'no-start']">
            <div class="goods-price-box">
              <span class="price">
                <en-price :price="goods.price" direction="row" size="24" />
                <en-price :price="goods.original_price" direction="row" lineThrough size="14" price-color="#999999" />
              </span>
...
export default {
  ...
  computed: {
    /** 获取当前选中的时间节点是否开始 */
    seckillIsStart() {
      let time = this.timeLines.filter(item => item.active)[0]
      return parseInt(time.distance_time) === 0
    }
  },
  mounted() {
    this.$nextTick(() => {
    ...
    GET_TimeLine() {
      API_Promotions.getSeckillTimeLine().then(response => {
        ...
        response.map((item, index) => {
          item.active = index === 0
          if (parseInt(item.distance_time) === 0 && index === 0) {
            if (onlyOne) {
...
.seckill-timeline {
  ...
  .timeline-item {
    ...
    text-align: center;
    font-size: 18px;
    &:hover:not(.active) .line-timeline {
  ...
  .timeline-item.active .main-timeline-item {
    width: 354px;
    &::before {
      border-color: transparent transparent theme.$color-main;
    }
    &::after {
      border-color: theme.$color-main transparent transparent;
    }
    .timeline-wrap {
      color: #fff;
...
.seckill-goods ::v-deep .el-pagination {
  text-align: right;
  button, li {
    background-color: #f6f6f6
  }
