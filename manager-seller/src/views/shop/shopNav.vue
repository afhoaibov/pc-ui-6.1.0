    ...
    <el-dialog title="新增导航" :visible.sync="addShopNavshow"  width="35%">
      <el-form :model="navform" :rules="rules" ref="navform" label-position="right" status-icon label-width="120px">
        <el-form-item label="导航名称:" prop="name">
          <el-input placeholder="名称长度最多为6个字符" v-model="navform.name" @change="()=> { navform.name = navform.name.trim() }" />
        </el-form-item>
      ...
      return {
        ...
        rules: {
          name: [
            ...
            { max: 6, message: '名称长度最多为6个字符', trigger: 'blur' },
            { whitespace: true, message: '导航名称不可为纯空格', trigger: 'blur' }
          ],
          sort: [
            { required: true, validator: shopNavSort, trigger: 'blur' }
          ],
          nav_url: [
...
<style type="text/scss" lang="scss" scoped>
  .shop-nav ::v-deep div.toolbar {
    height: 70px;
    padding: 20px 0;
  }

  .shop-nav ::v-deep .el-table td:not(.is-left) {
    text-align: center;
  }
  ...
  .shop-nav ::v-deep .el-input__inner{
    width: 60%;
  }
  span.URLtip {
    display: block;
