<template>
  <div class="full-cut">
    <en-table-layout
      ...
      <full-cut-preview slot="drawer" :promotionId="currentRow.id" />
    </en-table-layout>
  </div>
</template>
...
<script>
  ...
  import FullCutPreview from './components/FullCutPreview'
  ...
  export default {
    ...
    beforeRouteEnter(to, from, next) {
      next(vm => {
        if (from.name === 'addFullCut') {
          vm.GET_FullCutActivityList()
        }
      })
    },
    methods: {
      ...
      /** 跳转至新增满减活动页面 */
      handleAddFullCut() {
        this.$router.push({ name: 'addFullCut', params: { callback: this.GET_FullCutActivityList }})
      },

      /** 跳转至编辑满减活动页面 */
      handleEditMould(row) {
        this.$router.push({ name: 'editFullCut', params: { promotion_id: row.id, callback: this.GET_FullCutActivityList }})
      },

      /** 跳转至满减活动详情页面 */
      handleDetailMould(row) {
        this.$router.push({ name: 'checkFullDetail', params: { promotion_id: row.id, callback: this.GET_FullCutActivityList }})
      },
      ...
      /** 执行删除*/
      toDelActivity(row) {
        API_activity.deleteFullCutActivity(row.id).then(() => {
      ...
      /** 执行关闭 */
      toCloseActivity(row) {
        API_activity.closeFullCutActivity(row.id).then(response => {
...
<style type="text/scss" lang="scss" scoped>
  .full-cut ::v-deep div.toolbar {
    height: 70px;
    padding: 20px 0;
  }

  .full-cut ::v-deep .toolbar {
    padding: 15px;
  }
  .drawer-text {
    color: #006BB4;
