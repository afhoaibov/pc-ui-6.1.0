  ...
  <div>
    ...
    >
      ...
      <template slot="table-columns">
        ...
        <el-table-column prop="nav_url" label="URL"/>
        <el-table-column prop="target" label="新窗口">
    ...
    >
      ...
      <el-form :model="siteMenuForm" :rules="siteMenuRules" ref="siteMenuForm" label-position="right" status-icon label-width="120px">
        <el-form-item label="导航名称:" prop="name">
          <el-input placeholder="名称长度最多为6个字符" v-model="siteMenuForm.name" @change="()=> { siteMenuForm.name = siteMenuForm.name.trim() }" />
        </el-form-item>
        <el-form-item label="是否显示:" prop="disable">
          <el-radio-group v-model="siteMenuForm.disable">
            <el-radio :label="1">显示</el-radio>
            <el-radio :label="0">不显示</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="排序:" prop="sort">
          <el-input-number v-model="siteMenuForm.sort" controls-position="right" :min="0" :max="99999"/>
          <div class="URLtip">数字越大越靠前</div>
        </el-form-item>
        <el-form-item label="URL:" prop="nav_url">
          <el-input v-model="siteMenuForm.nav_url" placeholder="请输入URL" @change="() => { siteMenuForm.nav_url = siteMenuForm.nav_url.trim() }" ></el-input>
        ...
        <el-form-item label="新窗口打开:" prop="target">
          <el-radio-group v-model="siteMenuForm.target">
    ...
    methods: {
      ...
      handleDeleteSiteMenu(index, row) {
        this.$confirm('确定要删除这个导航菜单吗？', '提示', { type: 'warning' }).then(() => {
          API_SiteMenu.deleteSiteMenu(row.id).then(() => {
      ...
      submitSiteMenuForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            const { id } = this.siteMenuForm
            if (id) {
              API_SiteMenu.editSiteMenu(id, this.siteMenuForm).then(response => {
