...
function startWorker(accessToken, uuid) {
  let prefix = alias.admin || ''
  if (prefix && prefix.indexOf('/') !== 0) {
    prefix = `/${prefix}`
  }
  const worker = new SharedWorker(`${prefix}/static/js/worker.js`, 'shoptnt_admin')
  worker.port.addEventListener('message', function(e) {
    const data = JSONBig.parse(e.data.data)
    Vue.prototype.$eventCenter.emit(data.module_type, data.data)
  }, false)
  worker.port.postMessage({ cmd: 'open', url: api.admin + '/ws/shop', token: accessToken, uuid: uuid })
  worker.port.start()
