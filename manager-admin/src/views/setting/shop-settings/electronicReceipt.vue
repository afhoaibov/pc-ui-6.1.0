        ...
        <el-table-column label="启用状态">
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="text"
              @click="handleOpenElectronicReceipt(scope.$index, scope.row)"
            >开启</el-button>
          </template>
        </el-table-column>
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="primary"
              @click="handleEditElectronicReceipt(scope.$index, scope.row)"
            >设置</el-button>
          </template>
        </el-table-column>
      </template>
    ...
    >
      <el-form :model="electronicReceiptForm" label-width="100px">
        <el-form-item
          ...
          :label="config.text"
        >
        <template v-if="config.name === 'AppKey' || config.name === 'AppSecret'">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      /** 开启电子面单 */
      handleOpenElectronicReceipt(index, row) {
        API_ElectronicReceipt.openElectronicReceipt(row.bean).then(response => {
