  ...
  <div class="container">
    ...
    >
      <div slot="toolbar" class="inner-toolbar">
        <div class="toolbar-btns">
          <!-- <el-button type="primary" icon="el-icon-circle-plus-outline" @click="addNoticeTemplates">新增</el-button> -->
        </div>
    ...
    >
      <el-form :model="stationForm" :rules="stationFormRules" ref="stationForm" label-width="120px">
        <el-form-item label="邮件内容：" maxlength="100" prop="template_content">
          <el-input
            type="textarea"
  ...
  export default {
    ...
    data() {
      return {
        ...
        stationForm: {},
        stationFormRules: {
          template_content: this.MixinRequired('请输入邮件内容！')
        },
        // 弹窗title
        dialogTitle: '编辑邮件模板',
        // 弹窗显隐
    ...
    methods: {
      ...
      submitEvent() {
        this.$refs.stationForm.validate((valid) => {
          if (valid) {
            ...
            if (stationForm.id) {
              API_Message.updateMailTemplates(stationForm).then(res => {
      ...
      changeEnableStatus(row) {
        ...
        API_Message.changeMailTemplatesStatus(params).then(res => {
      ...
      getMailTemplates() {
        this.loading = true
        const { params } = this
        API_Message.getMailTemplates(params).then(response => {
          this.loading = false
