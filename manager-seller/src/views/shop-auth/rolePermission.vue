...
<script>
  ...
  // 涉及导出相关页面
  const exportIdentifier = ['orderList', 'serviceList', 'settlementManage']
  export default {
    name: 'rolePermission',
    data() {
      return {
    ...
    watch: {
      $route: {
        ...
        handler(newVal) {
          ...
          if (this.role_id === undefined || this.role_id === null) return
      ...
      setExportChecked(item) {
        ...
        if (item.checked && this.exportIdentifier.some(v => v === item.identifier)) {
          this.permissions.map((permissionsItem) => {
            if (permissionsItem.identifier === 'backstageTask') {
              permissionsItem.children[0].checked = true
            }
          })
        }
      },
