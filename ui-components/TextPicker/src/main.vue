...
<script>
  export default {
    ...
    methods: {
      handleConfirm() {
        let that = this
        this.$refs['textForm'].validate(function (valid) {
          if (valid) {
            that.$emit('confirm', that.textForm)
            that.$emit('close')
          } else {
            that.$message.error('表单填写有误，请核对！')
            return false
          }
        })
      }
...
<style type="text/scss" lang="scss" scoped>
  ...
  .text-picker-dialog ::v-deep .el-dialog__body {
    padding: 0 20px;
  }
  .text-picker-footer {
    padding-top: 15px;
