  ...
  >
    ...
    <template slot="table-columns">
      ...
      <el-table-column label="价格">
        <template slot-scope="scope">
          <span v-if="!scope.row.exchange_point">{{ scope.row.price | unitPrice }}</span>
          <span v-else>{{ scope.row.price | unitPrice }} + {{ scope.row.exchange_point }}积分</span>
        </template>
      </el-table-column>
    ...
    <el-pagination
      ...
      <goods-base-info
        slot="drawer"
        :goodsId="currentRow.draft_goods_id"
        isDraftGoods />
    </en-table-layout>
  </div>
</template>
    ...
    watch: {
      $route: {
        ...
        handler(newVal) {
          ...
          this.currentRow = {}
          if (newVal.name !== 'draftList') return
      ...
      GET_DraftGoodsList() {
        ...
        API_goods.getDraftGoodsList(this.params).then(response => {
          ...
          this.tableData.forEach(key => {
            let original = JSON.parse(key.original)
            key.original = original[0]
          })
        })
      },
...
<style type="text/scss" lang="scss" scoped>
  .draft-list ::v-deep .el-table td:not(.is-left) {
    text-align: center;
  }

  /** 工具条 */
  .draft-list ::v-deep div.toolbar {
    height: 70px;
    padding: 20px 0;
  }
  .inner-toolbar {
