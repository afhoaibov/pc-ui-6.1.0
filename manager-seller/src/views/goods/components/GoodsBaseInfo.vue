<template>
  <div class="goods-info-wrap">
    <div v-loading="loading" class="content-box" ref="goodsInfoWrap">
      ...
      <el-row :gutter="0">
        <el-col :span="24">
          ...
          <div class="d-content">
            ...
            <div class="item" v-if="!MixinIsSupplier()">
      ...
      <el-row :gutter="0">
        <el-col :span="24">
          ...
          <div class="d-content">
            ...
            <div class="item" v-if="goodsDetail.supplier_price">
                <span class="item-label">供应价格：</span>
                <span class="item-value">{{goodsDetail.supplier_price}}</span>
              </div>
            <div class="item" v-if="MixinIsSupplier()">
              <span class="item-label">供应价格：</span>
              <span class="item-value">{{goodsDetail.price}}</span>
            ...
            <div class="item">
              ...
              <video
                :src="goodsDetail.goods_video"
                v-if="goodsDetail.goods_video"
                class="video-avatar"
                :autoplay="false"
                controls>
                您的浏览器不支持视频播放
      ...
      <el-row :gutter="0" >
        <el-col :span="24">
          ...
          <div class="d-content">
            ...
            <el-table
              border
              v-if="selectTemplate.items && selectTemplate.items.length && Number(goodsDetail.template_id)"
    ...
    <div class="translation-btn">
      <el-dropdown size="medium" trigger="click" @command="dropdownEvent">
        ...
        <el-dropdown-menu slot="dropdown">
          ...
          <el-dropdown-item v-if="!MixinIsSupplier()">
    ...
    <sold-out-reason-dialog
      ...
      @handleConform="handleOperateGoods" />
  </div>
</template>
...
<script>
  ...
  import SoldOutReasonDialog from './SoldOutReasonDialog'
  ...
  export default {
      ...
      goodsId: {
        handler(value) {
          ...
          if (value) {
            ...
            if (this.isDraftGoods) {
              this.getGoodDraftData()
            } else {
              this.getGoodsDetail()
            }
          }
    ...
    methods: {
      ...
      dropdownEvent(command) {
        if (command === 'soldOut') {
          const market_enable = this.goodsDetail.market_enable
          if (market_enable === 0) {
      ...
      /** 获取草稿箱商品数据 */
      getGoodDraftData() {
        API_goods.getGoodDraftData(this.goodsId).then(res => {
      ...
      /** 查询商品详情 */
      getGoodsDetail() {
        API_goods.getGoodData(this.goodsId).then(res => {
          this.handleGoodsData(res)
        }).catch(() => {
          this.loading = false
        })
      ...
      getGoodsCategoryList() {
        API_goodsCategory.getGoodsCategoryList().then(res => {
          let goodsGroup = ''
          const defaultId = this.goodsDetail.shop_cat_id
          res.forEach(item => {
            if (item.shop_cat_id === defaultId) {
              goodsGroup = item.shop_cat_name
              return
            }
            item.children && item.children.forEach(child => {
              if (child.shop_cat_id === defaultId) {
      ...
      getGoodsBrandList() {
        ...
        API_goods.getGoodsBrandList(goodsDetail.category_id).then(response => {
          const item = response.find(item => { return goodsDetail.brand_id === item.brand_id })
          this.brand = item ? item.name : ''
        })
      },
      /** 获取运费模板数据 */
      getSimpleTpl() {
        API_express.getSimpleTpl(this.goodsDetail.template_id).then(res => {
      ...
      /** 商品参数获取 */
      async getEditGoodsParams() {
        let res = null
        if (this.isDraftGoods) {
          res = await API_goods.getGoodsDraftParams(this.goodsId)
        } else {
          res = await API_goods.getEditGoodsParams(this.goodsDetail.category_id, this.goodsId)
        }
