  ...
  [types.SET_CART_DATA](state, data) {
    data.forEach(shop => {
      shop.sku_list.forEach(sku => {
        if (sku.point_disable && sku.single_list && sku.single_list.length) {
...
export const getters = {
  ...
  skuList: state => {
    ...
    state.shopList.forEach((item) => {
      return _skuList.push(...item.sku_list)
    })
    return _skuList
  },
  /**
  ...
  */
  allCount: state => {
    ...
    state.shopList.forEach(shop => {
      ...
      if (shop.group_list && shop.group_list.length !== 0) {
        let group_sku_list = []
        shop.group_list.forEach(group => {
          group.sku_list.forEach(sku => {
            group_sku_list.push(sku)
          })
        })
        group_sku_list.forEach(item => {
          if (item.invalid !== 1) {
            _allCount += item.num
          }
        })
  ...
  checkedData: state => {
    ...
    state.shopList.forEach(shop => {
      shop.sku_list.forEach(item => {
        if (item.checked && item.invalid !== 1) {
          _checkedCount += item.num
    ...
    return { checkedCount: _checkedCount, checkedList: _checkedList }
  },
