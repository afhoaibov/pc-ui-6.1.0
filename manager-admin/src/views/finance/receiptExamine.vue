<template>
  <div class="container">
    <en-table-layout :tableData="tableData.data" size="small" style="width: 100%" :loading="loading">
      <div slot="toolbar" class="inner-toolbar">
        <div class="toolbar-btns"></div>
        <div class="toolbar-search">
          <en-table-search @search="searchEvent" @advancedSearch="advancedSearchEvent" advanced advancedWidth="500"
            placeholder="请输入会员名称">
      ...
      <template slot="table-columns">
        ...
        <el-table-column prop="taxpayer_code" label="纳税人识别号">
          <template slot-scope="{ row }">
            <clipboard :text="row.taxpayer_code" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button size="mini" type="primary" @click="handlViewExamineDetail(scope.row)">{{ scope.row.status ===
              'NEW_APPLY' ? '审核' : '查看' }}</el-button>
          </template>
        </el-table-column>
      </template>
    ...
    <!--审核 dialog-->
    <el-dialog :title="detailsData.status === 'NEW_APPLY' ? '增票资质审核' : '增票资质详情'" :visible.sync="dialogReceiptExamine"
      ...
      <div class="receipt-examine">
        ...
        <p>
          纳税人识别号：
          <clipboard :text="detailsData.taxpayer_code" />
        </p>
        <p>注册地址：{{ detailsData.register_address }}</p>
        <p>
          注册电话：
          <clipboard :text="detailsData.register_tel" />
        </p>
        <p>开户银行：{{ detailsData.bank_name }}</p>
        <p>银行账户：{{ detailsData.bank_account }}</p>
        <p v-if="detailsData.status !== 'NEW_APPLY'">审核状态：{{ detailsData.status === 'AUDIT_PASS' ? '通过' : '未通过' }}</p>
        <p style="word-break:break-word;" v-if="detailsData.status === 'AUDIT_REFUSE'">备注信息：{{ detailsData.audit_remark }}
        </p>
      </div>
      <el-form :model="formExamine" :rules="authInfoRules" ref="formExamine" v-if="detailsData.status === 'NEW_APPLY'"
        class="receipt-form">
        ...
        <el-form-item label="备注信息" prop="remark">
          <el-input type="textarea" v-model="formExamine.remark" placeholder="请输入审核备注（120字以内）" maxlength="120"
...
export default {
  ...
  watch: {
    $route: {
      immediate: true,
      handler(newVal) {
        if (newVal.name !== 'receiptExamine') return
