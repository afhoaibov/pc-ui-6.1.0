    ...
    methods: {
      ...
      GET_OrderStatisticsOrder() {
        ...
        API_Statistics.getOrderStatisticsOrder(this.params).then(response => {
          ...
          this.echarts = echarts.init(document.getElementById('order-statistics-order-chart'))
          this.echarts.setOption(echartsOptions({
            color: ['#c23531', '#2f4554'],
            titleText: `订单下单量统计（${cycle_type === 'MONTH' ? '月' : '年'}）`,
            tooltipFormatter: (params) => {
              return `日期：${params[0].dataIndex + 1}${type_str}<br/>
                     ${params[0] ? `${params[0].marker}${params[0].seriesName}下单数量：${params[0].value}` : ''} <br/>
                     ${params[1] ? `${params[1].marker}${params[1].seriesName}下单数量：${params[1].value}` : ''}`
            },
            legend: {
              right: 50,
